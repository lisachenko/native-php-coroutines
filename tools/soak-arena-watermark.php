 ...
 * - **result-slot consumption**, which is a bump allocation of its own: slots are taken from a
 *   pre-sized table and never given back, so the soak reports how many of them a round costs and
 *   sizes the table for the run rather than pretending the supply is endless;
...
/** @return array{rounds: int, tasks: int, warmup: int, tolerance: int, workers: int, selfTest: bool} */
function options(): array
{
    $parsed = getopt('', ['rounds:', 'tasks:', 'warmup:', 'tolerance:', 'workers:', 'self-test', 'help']) ?: [];

    if (array_key_exists('help', $parsed)) {
        echo 'usage: soak-arena-watermark.php [--rounds=40] [--tasks=8] ',
        '[--warmup=<a quarter of the rounds>] [--tolerance=4096] [--workers=2] [--self-test]', PHP_EOL;
...
// Result slots are bump-allocated from a pre-sized table and never given back, so a run that asks
// for more than the table holds fails on exhaustion rather than on the thing it set out to measure.
// Refusing up front is more useful than discovering it two hundred rounds in.
$slotCount   = SharedArena::DEFAULT_SLOT_COUNT;
$slotsNeeded = $options['rounds'] * $options['tasks'];

if ($slotsNeeded > $slotCount) {
    printf(
        'SOAK arena-watermark: INCONCLUSIVE — %d rounds of %d tasks need %d result slots and the ' .
        'table holds %d; lower --rounds or --tasks%s',
        $options['rounds'],
        $options['tasks'],
        $slotsNeeded,
        $slotCount,
...
$runtime = new Runtime(workers: $options['workers'], arenaSize: 64 << 20);
$arena   = $runtime->arena();
...
' tasks on ', $options['workers'], ' workers, first ', $options['warmup'], ' discarded as warmup',
...
$runtime->run(static function (RuntimeInterface $self) use (
    ...
    for ($round = 0; $round < $options['rounds']; ++$round) {
        ...
        printf(
            'round %3d  watermark %12s  allocated %10s  rss %10s%s',
            $round + 1,
    ...
    $slotsConsumed = $options['rounds'] * $options['tasks'];
});
...
echo PHP_EOL;
printf(
    'result slots consumed: %d of %d reserved — bump-allocated and never given back%s',
    $slotsConsumed,
    $slotCount,
    PHP_EOL,
);
