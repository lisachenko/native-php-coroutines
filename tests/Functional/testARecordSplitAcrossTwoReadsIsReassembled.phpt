--TEST--
A control record delivered in two pieces is held back until all 32 bytes are there
--INI--
ffi.enable=1
opcache.jit=off
...
fwrite($writer->stream(), substr($frame, 0, 20));

$firstBatch = [];
parallelWaitFor(static function () use ($reader, &$firstBatch): bool {
    $firstBatch = array_merge($firstBatch, $reader->drain());

    return $reader->pendingBytes() === 20;
}, 2.0);

echo 'records after 20 bytes: ', count($firstBatch), "\n";
echo 'bytes held back: ', $reader->pendingBytes(), "\n";

fwrite($writer->stream(), substr($frame, 20));
...
// Two records written as one 64-byte blob come back as two, not as one oversized frame.
