...
preferences: the preemption ones come from `spikes/` (see [`spikes/README.md`](spikes/README.md);
note that the `VERDICTS.md` and `raw/` it points at are **not currently committed** — until they are,
the verdict summary lives in the message of the commit that added the spikes), the shared-memory ones
from the substrate spikes in
